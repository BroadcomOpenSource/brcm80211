...
printk("dma_attach: ntxd=%d nrxd=%d\n", ntxd, nrxd);
	...
	printk("%s: dma_attach: %s flags 0x%x ntxd %d nrxd %d "
	...
	if (di->aligndesc_4k) {
		printk("dma_attach: aligndesc_4k\n");
	...
	printk("ddoffsetlow 0x%x ddoffsethigh 0x%x dataoffsetlow 0x%x dataoffsethigh " "0x%x addrext %d\n", di->ddoffsetlow, di->ddoffsethigh, di->dataoffsetlow, di->dataoffsethigh, di->addrext);
				    ...
				    bool commit)
{
 ...
 outoftxd:
	printk("%s: dma_txfast: out of txds !!!\n", di->name);
...
static void *dma64_getnexttxp(dma_info_t *di, txd_range_t range)
{
 ...
 bogus:
	printk("dma_getnexttxp: bogus curr: start %d end %d txout %d\n", start, end, di->txout);
	return NULL;
}
...
void dma_dump_channel(struct hnddma_pub *dmah)
{
	struct dma_info *di = (struct dma_info *) dmah;
	dma64regs_t *p = di->d64txregs;
	printk("dma_dump_channel @%p: control=%08X ptr=%08X addrlow=%08X addrhigh=%08X status0=%08X status1=%08X\n",
		&p->control,
		R_REG(&p->control), R_REG(&p->ptr), R_REG(&p->addrlow), R_REG(&p->addrhigh),
		R_REG(&p->status0), R_REG(&p->status1) );
