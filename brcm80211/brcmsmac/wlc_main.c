...
void dbg_dump(struct wlc_info *wlc)
{
	int i;
        printk("pktqlen=%d TXPKTPENDTOT=%d macintmask=%X macintstatus=%X up=%d hw_off=%d hw_up=%d radio_disabled=%d going_down=%d check_rad_dis=%d\n",
wlc->pkt_queue->q.len, TXPKTPENDTOT(wlc),
R_REG(&wlc->hw->regs->macintmask), R_REG(&wlc->hw->regs->macintstatus),
wlc->pub->up, wlc->pub->hw_off, wlc->pub->hw_up,
wlc->pub->radio_disabled, wlc->going_down, wlc_check_radio_disabled(wlc));
	for (i=0;i<4;i++) {
		printk("%s: fifo=%d intmask=0x%08X intstat=0x%08X\n", __func__, i, R_REG(&wlc->regs->intctrlregs[i].intmask), R_REG(&wlc->regs->intctrlregs[i].intstatus));
		dma_dump_channel(wlc->hw->di[i]);
	}
}
...
printk("wlc_wait_for_tx_completion: entry\n");
	dbg_dump(wlc);                
        if (drop) {
printk("drop\n");
...
printk("/* wait for queue and DMA fifos to run dry */\n");
